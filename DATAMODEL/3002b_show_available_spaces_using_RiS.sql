-- create view to show available spaces

drop materialized view IF EXISTS demand."Demand_view_to_show_available_spaces" CASCADE;

create MATERIALIZED VIEW demand."Demand_view_to_show_available_spaces"
...
AS
    ...
    case when f."Demand" >= sv."NrBays" then 0
         else sv."NrBays" - f."Demand"
    end as "NrBays",
    f."Capacity",
	...
	AND f."SurveyID" = sv."SurveyID"
	
with DATA;

alter table demand."Demand_view_to_show_available_spaces"
    OWNER TO postgres;

create UNIQUE INDEX "idx_Demand_view_to_show_available_spaces_id"
    ON demand."Demand_view_to_show_available_spaces" USING btree
    (id)
    TABLESPACE pg_default;

REFRESH MATERIALIZED VIEW demand."Demand_view_to_show_available_spaces";
