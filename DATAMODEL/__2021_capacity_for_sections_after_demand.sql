...
BEGIN
    ...
    LOOP
        ...
        FOR relevant_count IN
            SELECT "SurveyID", "GeometryID",
               SUM(COALESCE("NrCars"::float, 0.0) +
                ...
                COALESCE("NrTaxis_Suspended"::float, 0)) As "Demand on YL"
               FROM demand."Counts"
               WHERE "GeometryID" = relevant_restriction."GeometryID"
               GROUP BY "SurveyID", "GeometryID"
        LOOP

            IF relevant_count."Demand in Bays" + relevant_count."Demand on YL" > 0 THEN
                zero_capacity = false;
                EXIT;
            END IF;
		...
		ELSE
		    ...
		    UPDATE mhtc_operations."Supply"
            SET "CapacityFromDemand" = "Capacity"
            WHERE "GeometryID" = relevant_restriction."GeometryID";
